...
# - "gauss_fullcov" (default): current method, adapt full covariance Gaussian RW then normalize c
...
filename_data = os.path.join(data_dir, f"k{kappa_actual}_n{n_points}_data.pkl")
...
filename_results = os.path.join(result_dir, f'results_k{kappa_actual}_n{n_points}.csv')
filename_runs = os.path.join(result_dir, f'runs_k{kappa_actual}_n{n_points}.csv')
...
def run_simulation(sim_id):
    ...
    try:
        ...
        c_optimals = np.array(solve_io_all(data_dict[sim_id], r), dtype=float)
        result = run_mcmc_var(sim_id, r, k, c_optimals, dim, c_actual, kappa_actual,
